...
) : Component(transform)
{
    ...
    if (MeshRenderer::meshNames.find(this->meshName) == MeshRenderer::meshNames.end()) {
        MeshRenderer::meshNames.insert(this->meshName);
        MeshFactory();
    }
}
...
void MeshRenderer::LateUpdate(const float deltaTime)
{
    if (debugOnly && !scene->GetDebugMode()) {
        return;
    }

    // Check if the mesh's layer is included in the current camera's layers
    if (!GameManager::GetInstance()->GetSceneReference()->GetCurrentCamera()->IsLayerRendered((int)layer)) {
        return;
    }

    glm::mat4 finalModelMatrix = transform->GetModelMatrix() * scaleMatrix;

    scene->DrawMesh(this->meshName, shader, finalModelMatrix, renderUI, texture, texScale);
}

void MeshRenderer::SetScale(glm::vec3 newScale)
{
    this->scaleMatrix = Transform::GetScalingMatrix(newScale);
}
...
void MeshRenderer::SetColor(glm::vec4 newColor)
{
    ...
    if (MeshRenderer::meshNames.find(newMeshName) == MeshRenderer::meshNames.end()) {
        MeshRenderer::meshNames.insert(newMeshName);
...
void MeshRenderer::SetTexture(std::string newTexture, float newTexScale)
{
    texture = newTexture;
...
void MeshRenderer::MeshFactory()
{
    ...
    mesh->SetDrawMode(meshDescription.drawMode);
    scene->GenerateMesh(mesh, meshDescription.vertices, meshDescription.indices);
}

void MeshRenderer::LoadMesh(const std::string name, const std::string path)
{
    generateMesh = false;
    scene->LoadMesh(meshName, name, path);
}
